...
###### New Zealand ########  
  ...
  nz <- sf::st_as_sf(nz)
  nz <- cbind(nz, st_coordinates(st_centroid(nz)))
  nz_regions <- sf::st_as_sf(nz_regions)
  nz_regions <- cbind(nz_regions, st_coordinates(st_centroid(nz_regions)))
...
###### Australia ########  
  ...
  #### Convert to SF objects and generate centroids
  aus <- sf::st_as_sf(aus)
  aus <- cbind(aus, st_coordinates(st_centroid(aus)))
  aus_regions <- sf::st_as_sf(aus_regions)
  aus_regions <- sf::st_make_valid(aus_regions)
  # aus_regions<- cbind(aus_regions, st_coordinates(st_centroid(aus_regions)))
...
###### Grab CountryName from Devices (New Zealand -> NZ; Australia -> AU, Ireland -> IE; etc. )
  ...
  countryCode <- countrycode(countryName, origin = 'country.name', destination = 'iso2c')
...
###### Function takes country spatial files ,  internal boundaries and connected data and produces output map.
  ...
  ### MAIN MAP
      main_map <- country %>%
        ...
        geom_point(data = country_points, aes(x = long, y = lat,colour = connected),size=0.7)+
...
Of `r alldevice_n` deployed devices, `r included_n` (shown in green) connected successfully, with approximate locations. Those in red were connected less than 25% of the time, so are excluded from the analysis.  `r devices_no_coords_n` devices were excluded due to missing location data.
