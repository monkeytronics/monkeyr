...
tryCatch(
  {
    ...
    sf::sf_use_s2(FALSE)
    message(paste0("\nMESSAGE OUT : After library loading "))
    ...
    ###### Function takes country spatial files ,  internal boundaries and connected data and produces output map.
      ...
      ### MAIN MAP
          main_map <- country %>%
            ...
            geom_point(data = country_points, aes(x = long, y = lat,colour = connected),size=0.5)+
...
Of `r count_all` deployed devices, `r count_connected` (shown in green) connected successfully, with approximate locations. Those in red were connected less than 10% of the time, so are excluded from the analysis.  `r count_no_coords` devices could not be shown on the map due to missing location data.
