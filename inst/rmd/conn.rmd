...
params:
  filtered_obs:             "../dummy-data/conn_6December/obs.csv"
  filtered_weather:         "../dummy-data/conn_6December/weather.csv"
  filtered_devices:         "../dummy-data/conn_6December/dev.csv"
  filtered_interventions:   "../dummy-data/conn_6December/interv.csv"
  report_params:            "-map-"                                            
  ...
  tryCatch(
    {
      ...
      # timespan <- params$toTimeStamp - params$fromTimeStamp ## use maxTs & minTs instead!
      ...
      wrangled_devices_2 <-
        ...
        dplyr::summarise(
          newest_mgs_ts = max(ts),
          temp = max(temp),
          data_vol = n() / fullvol
          ) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(
          newest_mgs_ts = tidyr::replace_na(newest_mgs_ts, 0)
        ) %>%
        dplyr::mutate(conn = (maxTs - newest_mgs_ts) < (3600*24*2)) %>%   ## last 2 days
        dplyr::select(-c(device_id, newest_mgs_ts, country))
      ...
      wrangled_devices_3 <-
        wrangled_devices_2 %>%
        ## Running latest Firmware & connected - Assume cert rotation complete! (OK)
        dplyr::mutate(ok = (fw_ver == LATEST_FW) & conn) %>%
        ## 1) NEVER connected (Failed Install)
        dplyr::mutate(nc = (last_comm == 0 | last_comm == 1681892100)) %>%
        ## 2) have connected, but did not get latest firmware (need OFF/ON)
        dplyr::mutate(offline = (fw_ver < LATEST_FW & (!nc))) %>%
        ## 3) switched off at mains (but has latest FW and prob. certs)
        dplyr::mutate(power_off = (fw_ver == LATEST_FW) & !conn) %>%
        
        ## offline -> add to off_on list
        dplyr::mutate(offline_list = case_when(
          offline  ~ paste0(" -", classRoom, "- "),
          !offline ~ ""
        )) %>%
        
        ## never_connected -> nc list
        dplyr::mutate(nc_list = case_when(
          nc  ~ paste0(" -", classRoom, "- "),
          !nc ~ ""
        )) %>%
        
        ## power_off -> power_off list
        dplyr::mutate(power_off_list = case_when(
          power_off  ~ paste0(" -", classRoom, "- "),
          !power_off ~ ""
        )) %>%
        ...
        dplyr::summarise(
          ...
          # temp = first(temp),
          ok = sum(ok),
          not_ok = sum(offline) + sum(nc) + sum(power_off),
          offline = sum(offline),
          nc = sum(nc),
          power_off = sum(power_off),
          offline_list = paste0(offline_list, collapse = ""),
          nc_list = paste0(nc_list, collapse = ""),
          power_off_list = paste0(power_off_list, collapse = "")
          ) %>%
        dplyr::ungroup() %>%
        dplyr::arrange(desc(offline))
      ...
      kable_summary <-
        wrangled_devices_3 %>%
        ## Add Column Totals on Numeric
        summarise(across(where(is.numeric), sum)) %>% 
        bind_rows(., wrangled_devices_3) %>% 
        select(school_name, everything()) %>%
        ## Format Kable
        kableExtra::kbl(format = "html", align = "lcccccrr") %>%
        kableExtra::kable_material(lightable_options = c("striped", "hover", "condensed"), html_font = "sans-serif") %>%
        kableExtra::column_spec(1:5, width = c("25em","17em","17em","17em","17em","35em"))
...
## Will Do this inside AWS Lambda - this is a horrible place to write this code!
    ...
    error = function(cond) {
      monkeyr::monkey_knit_error(err = cond, resource = "duplicate_installs")
    }
  )
...
```{r test_devices, out.width='80%', fig.align="center", fig.height=3}
...
## Print Table
    ...
    error = function(cond) {
      monkeyr::monkey_knit_error(err = cond, resource = "test_devices")
    }
  )
```


<!-- # Device Hardware Issues {data-icon="fa-refresh"} -->
...
## Will Do this inside AWS Lambda - this is a horrible place to write this code!
    ...
    error = function(cond) {
      monkeyr::monkey_knit_error(err = cond, resource = "duff_installs")
    }
  )  
```

# Zero Readings Heat Maps

## Zero Readings 1

```{r zero_readings_1, out.width='150%', fig.align="center", fig.height=3}

  bin_width <- 10

  tryCatch(
    {
## Get Expiry for each device sn range < 200000
      zero_readings <-
        wrangled_devices_1 %>%
        dplyr::group_by(school_name, classRoom, device_id, fw_ver) %>%
        dplyr::summarise(
          temp = max(temp)
          ) %>%
        dplyr::filter(temp == 0) %>%
      ...
      zero_histogram_1 <-
        ...
        geom_histogram( binwidth=bin_width, fill="#4953f2", color="#e9ecef", alpha=0.4, just = 0) +
        ggtitle("Zero Readings S00XXXX Range") +
        theme_ipsum() +
        theme(
          ...
          plot.margin = margin(
            ...
            b = 10,  # Bottom margin
            l = 10)) + # Left margin
        scale_x_continuous(breaks=seq(floor(min(zero_readings$device_id)/bin_width)*bin_width, ceiling(max(zero_readings$device_id)/bin_width)*bin_width, by=10*bin_width)) + # Adjust 'by' for resolution
  ...
  ## Print Table
    ...
    error = function(cond) {
      monkeyr::monkey_knit_error(err = cond, resource = "zero_readings")
    }
  )  
```
...
```{r zero_readings_2, out.width='150%', fig.align="center", fig.height=3}
  tryCatch(
    {
## Get Expiry for each device sn range > 200000
      zero_histogram_2 <-
        ...
        ggplot2::ggplot( aes(x=device_id)) +
        geom_histogram( binwidth=bin_width, fill="#4953f2", color="#e9ecef", alpha=0.4, just = 0) +
        ggtitle("Zero Readings S20XXXX Range") +
        theme_ipsum() +
        theme(
          plot.title = element_text(size=6),
          axis.title = element_text(size = 3),
