...
tryCatch(
  ...
  ### 0. Libraries (Shared)
    #library(rgdal);        message(paste0("\nMAPPING : rgdal"))  ## not ACTUALLY Needed?!
    ...
    #library(ggspatial);    message(paste0("\nMAPPING : ggspatial "))  ## not ACTUALLY Needed?!
  ...
  ### 4. Draw Map
    ...
    error = function(cond) {
      # monkeyr::monkey_knit_error(err = cond, resource = "mapping", debug = TRUE)    ## Completely Bail!
      monkeyr::monkey_knit_msg(msg = cond, resource = "mapping", debug = TRUE)        ## Handle Error
      mapFailed <- TRUE
  }
)
